...
export default function FreelancerCommitmentsPage() {
        ...
        <div className="max-w-6xl mx-auto px-4 py-8">
            ...
            {commitments.length === 0 ? (
                <div className="bg-white rounded-xl border border-[#CCBEB1]/30 p-12 text-center">
                    <p className="text-[#8A796E]">No commitments assigned to you yet.</p>
                </div>
            ) : (
                <div className="space-y-4">
                    {commitments.map((commitment) => {
                        ...
                        // Check if there's an action available for this commitment
                        const canLock = commitment.status === "paid";
                        const canDeliver = commitment.status === "locked";
                        const hasAction = canLock || canDeliver;

                        return (
                            <Link
                                key={commitment.id}
                                href={`/dashboard/freelancer/commitments/${commitment.id}`}
                                className={`block bg-white rounded-xl border p-6 hover:shadow-md transition-shadow ${hasAction ? "border-[#997E67] border-2" : "border-[#CCBEB1]/30"
                                    }`}
                            >
                                <div className="flex items-start justify-between gap-4">
                                    <div className="flex-1 min-w-0">
                                        <div className="flex items-center gap-3 mb-2">
                                            ...
                                            {hasAction && (
                                                <span className="px-2 py-1 text-xs font-semibold rounded-full bg-[#997E67] text-white">
                                                    Action Required
                                                </span>
                                            )}
                                        </div>
